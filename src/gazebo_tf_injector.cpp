...
namespace gazebo {
    void GazeboTfInjector::Load(physics::ModelPtr model, sdf::ElementPtr sdf) {
        ...
        // Create a list of all links in the model.
        ...
        // Check if the model comprises at least one link.
        ...
        // Retrieve the TF frame that is the parent of the model. To that end,
        // start at the first link of the model and go up the TF hierarchy until
        // the current frame is no longer a link of the model.
        ROS_INFO_STREAM("Retrieving parent TF frame of model \"" 
            << this->model->GetName() << "\" ...");
        std::string frame = this->childLinks.begin()->first;
        while (this->childLinks.count(frame) > 0) {
            if (!this->tfListener->getParent(frame, ros::Time(0), frame)) {
                ROS_INFO_STREAM_ONCE("Waiting for parent frame of frame \"" 
                    << frame << "\" to become available ...");
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
            }
        }
        this->parentFrame = frame;
        ROS_INFO_STREAM("Found parent frame of model \"" 
            << this->model->GetName() << "\": \"" 
            << this->parentFrame << "\".");

        // Make the model static in order to prevent the physics engine to
        // change the injected transformations.
        this->model->SetStatic(true);

        // Register the callback that updates the model poses every time the
        // simulated world is rendered.
        this->callback = event::Events::ConnectPreRender(
        ...
        for (std::pair<std::string, physics::LinkPtr> link : this->childLinks) {
            ...
            try {
                this->tfListener->lookupTransform(
                    this->parentFrame, link.first, ros::Time(0), transform);
